...
CREATE UNIQUE INDEX profiles_student_id_idx ON profiles(student_id);
```

`student_id` is automatically generated by the app when a profile is first
created using this format:

`HOPECOSA-<YEAR_OF_COMPLETION>-<RANDOM_NUMBER>`
...
3. Try signing up a new user in your app and confirm `profiles.student_id` was
   generated (for example: `HOPECOSA-2026-1234567890`)
